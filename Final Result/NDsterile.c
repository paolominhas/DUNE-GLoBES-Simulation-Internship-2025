...
char MYFILE3[]="ND_chi_final_extra.dat";
...
{ 
  ...
  glbInit(argv[0]); // Initialize GLoBES library (libglobes), multi experiment smeared-source
    for (int i = 1; i < 12; i++) {
      char glb_filename[128];
      sprintf(glb_filename, "nd_globes/ND%d.glb", i);
      glbInitExperiment(glb_filename, &glb_experiment_list[0], &glb_num_of_exps); // Initialize DUNE Near Detector experiment
    }
    glbInitExperiment("DUNE_GLoBES.glb", &glb_experiment_list[0], &glb_num_of_exps); // Initialize DUNE Far Detector experiment
  ...
  // Sterile χ² Output ND// 
    ...
    for (double en24 = -5.0; en24 <= -0.1; en24 += 0.1) {
        ...
        for (double enstdm = -4.0; enstdm <= 2.0; enstdm += 0.1) {
            ...
            chi_now = glbChiNP(test_values, NULL, GLB_ALL); // Use glbChiSys for clarity on which rule to use

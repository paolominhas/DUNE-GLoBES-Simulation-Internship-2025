...
char AEDLFILE[]="TOYdraft_resposta.glb";
...
{ 
 ...
 /* Defina os valores "true" para os parâmetros de oscilação (valores retirados de NuFit5.1 para ordenamento normal de massas)  */
  ...
  double *rule_nue  = glbGetRuleRatePtr(0, 1);
  double *rule_anue  = glbGetRuleRatePtr(0, 3);
  ...
  /* Calcule o chi^2 para theta_13 e delta_cp assumindo todos os outros parâmetros de oscilação fixos */
  ...
  double delta_upper = 340*M_PI/180.;
  ...
  for(this_th13=th13_lower; this_th13<=th13_upper; this_th13+=(th13_upper-th13_lower)/th13_steps)
  {
    for(this_delta=delta_lower; this_delta<=delta_upper; this_delta+=(delta_upper-delta_lower)/delta_steps)
    {
      ...
      res = glbChiSys(test_values, 0, 0) + glbChiSys(test_values, 0, 2);
...
/*
  for(this_th13=th13_lower; this_th13<=th13_upper; this_th13+=(th13_upper-th13_lower)/th13_steps)
  {
    for(this_delta=delta_lower; this_delta<=delta_upper; this_delta+=(delta_upper-delta_lower)/delta_steps)
    {
      ...
      double chi_exp0 = glbChiSys(test_values, 0, 0);
      double chi_exp2 = glbChiSys(test_values, 0, 2);
      double chi_total = chi_exp0 + chi_exp2;

      fprintf(outfile, "%g %g %g %g %g\n",
        this_th13 * 180.0 / M_PI,
        this_delta * 180.0 / M_PI,
        chi_exp0,
        chi_exp2,
        chi_total);
    }

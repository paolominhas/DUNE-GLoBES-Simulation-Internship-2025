...
{ 
  ...
  double pMatme = 0.0;
    ...
    for(en = 0.01; en<=1.2; en = en + (1.2 - 0.2)*0.01)
    {
    pVacmm = glbVacuumProbability(2,2,+1,en,295);
    pVacme = glbVacuumProbability(2,1,+1,en,295);
    pMatmm = glbProfileProbability(0,2,2,+1,en);
    pMatme = glbProfileProbability(0,2,1,+1,en);
    
    
    fprintf(outfile,"%g %g %g %g %g\n", en, pVacmm, pMatmm, pVacme, pMatme); 
    }
